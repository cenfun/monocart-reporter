// Match one tag at a time. The next @ or whitespace starts a new token.
// Playwright only requires a tag to start with @, so keep other characters.
export const tagPattern = /@([^@\s]+)/g;

export const getTagKeys = (tags) => {
    const keys = [];
    const list = tags instanceof Array ? tags : [tags];
    list.forEach((tag) => {
        const matches = `${tag}`.matchAll(tagPattern);
        for (const match of matches) {
            keys.push(match[1]);
        }
    });
    return keys;
};

export const attachments = {
    audit: {
        name: 'audit',
        contentType: 'text/html',
        reportFile: 'audit-report.json'
    },
    coverage: {
        name: 'coverage',
        contentType: 'text/html',
        reportFile: 'coverage-report.json'
    },
    network: {
        name: 'network',
        contentType: 'text/html',
        reportFile: 'network-report.json'
    },
    metadata: {
        name: 'metadata',
        contentType: 'application/json'
    }
};

export const pageTimings = [{
    key: 'onContentLoad',
    name: 'Content Loaded',
    color: '#1a1aa6'
}, {
    key: 'onLoad',
    name: 'Page Loaded',
    color: '#c80000'
}];

export const timings = [{
    key: 'blocked',
    name: 'Blocking',
    color: '#858585'
}, {
    key: 'dns',
    name: 'DNS Lookup',
    color: '#009688'
}, {
    key: 'connect',
    name: 'Connecting',
    color: '#b52dcd'
}, {
    key: 'send',
    name: 'Sending',
    color: '#74979a'
}, {
    key: 'wait',
    name: 'Waiting',
    color: '#00a846'
}, {
    key: 'receive',
    name: 'Receiving',
    color: '#0299de'
}];

export const hasOwn = (obj, key) => {
    return Object.prototype.hasOwnProperty.call(obj, key);
};

export const isNull = (input) => {
    if (input === null || typeof input === 'undefined') {
        return true;
    }
    return false;
};

export const uid = (len = 20, prefix = '') => {
    const dict = '0123456789abcdefghijklmnopqrstuvwxyz';
    const dictLen = dict.length;
    let str = prefix;
    while (len--) {
        str += dict[Math.random() * dictLen | 0];
    }
    return str;
};

export const zero = (s, l = 2) => {
    s = `${s}`;
    return s.padStart(l, '0');
};

export const isNum = (num) => {
    if (typeof num !== 'number' || isNaN(num)) {
        return false;
    }
    const isInvalid = (n) => {
        return n === Number.MAX_VALUE || n === Number.MIN_VALUE || n === Number.NEGATIVE_INFINITY || n === Number.POSITIVE_INFINITY;
    };
    if (isInvalid(num)) {
        return false;
    }
    return true;
};

export const toNum = (num, toInt) => {
    if (typeof (num) !== 'number') {
        num = parseFloat(num);
    }
    if (isNaN(num)) {
        num = 0;
    }
    if (toInt) {
        num = Math.round(num);
    }
    return num;
};

export const isList = (data) => {
    if (data && data instanceof Array && data.length > 0) {
        return true;
    }
    return false;
};

export const forEach = (rootList, callback) => {
    const isBreak = (res) => {
        return res === 'break' || res === false;
    };
    const forList = (list, parent) => {
        if (!isList(list)) {
            return;
        }
        for (const item of list) {
            const result = callback(item, parent);
            if (isBreak(result)) {
                return result;
            }
            const subResult = forList(item.subs, item);
            if (isBreak(subResult)) {
                return subResult;
            }
        }
    };
    forList(rootList);
};

// \ to /
export const formatPath = (str) => {
    if (typeof str === 'string') {
        str = str.replace(/\\/g, '/');
    }
    return str;
};

export const getCurrentTrendInfo = (data) => {
    const {
        date, duration, summary
    } = data;

    const info = {
        date,
        duration
    };

    Object.keys(summary).forEach((k) => {
        const item = summary[k];
        info[k] = item.value;
    });

    return info;
};

export const isTagItem = (item) => {
    // new syntax in playwright v1.42
    if (item.tags) {
        return true;
    }

    if (item.type === 'case' || (item.type === 'suite' && item.suiteType === 'describe')) {
        return true;
    }
    return false;
};

export const delay = (ms) => {
    return new Promise((resolve) => {
        if (ms) {
            setTimeout(resolve, ms);
        } else {
            setImmediate(resolve);
        }
    });
};

// =============================================================================

export const generatePercentChart = (percent) => {
    return `<div style="--mcr-percent:${percent}%;" class="mcr-percent-chart"></div>`;
};

export const getStatus = (value, watermarks) => {
    if (!watermarks) {
        return 'unknown';
    }
    if (value < watermarks[0]) {
        return 'low';
    }
    if (value < watermarks[1]) {
        return 'medium';
    }
    return 'high';
};

export const isJsonType = (contentType) => {
    if (contentType) {
        if (contentType === 'application/json' || contentType === 'json') {
            return true;
        }
    }
    return false;
};

export const isMarkdownType = (contentType) => {
    if (contentType) {
        if (contentType === 'text/markdown' || contentType === 'markdown') {
            return true;
        }
    }
    return false;
};

export const isMermaidType = (contentType) => {
    if (contentType) {
        if (contentType === 'text/vnd.mermaid' || contentType === 'text/mermaid' || contentType === 'mermaid') {
            return true;
        }
    }
    return false;
};

export const isTextType = (contentType) => {
    if (contentType) {
        if (contentType.startsWith('text')) {
            return true;
        }
        if (isMarkdownType(contentType)) {
            return true;
        }
        if (isMermaidType(contentType)) {
            return true;
        }
        if (isJsonType(contentType)) {
            return true;
        }
    }
    return false;
};

export const isLink = (str) => {
    if (typeof str === 'string') {
        const pattern = /^(https?|ftp|file|mailto|tel|data):\S+$/;
        return pattern.test(str);
    }
    return false;
};

// =============================================================================
// svg

export const dFixed = (num, fixed = 1) => {
    if (Number.isInteger(num)) {
        return num;
    }
    return toNum(toNum(num).toFixed(fixed));
};

export const pxFixed = (num) => {
    const floor = Math.floor(num);
    if (num < floor + 0.5) {
        return floor + 0.5;
    }
    return floor + 1.5;
};

export const point = (px, py) => {
    return `${dFixed(px)},${dFixed(py)}`;
};

// =============================================================================
// formatter

// number
export const NF = (v) => {
    if (typeof v === 'number' && v) {
        return v.toLocaleString();
    }
    return v;
};

// percent
export const PF = (v, t = 1, digits = 1, unit = '%', space = '') => {
    v = toNum(v);
    t = toNum(t);
    let per = 0;
    if (t) {
        per = v / t;
    }
    const perStr = (per * 100).toFixed(digits);
    if (unit) {
        return perStr + space + unit;
    }
    return parseFloat(perStr);
};

export const PSF = (v, t = 1, digits = 1) => {
    return PF(v, t, digits, '%', ' ');
};

export const PNF = (v, t = 1, digits = 1) => {
    return PF(v, t, digits, '');
};

// byte
export const BF = (v, places = 1, space = '') => {
    v = toNum(v, true);
    if (v === 0) {
        return `0${space}B`;
    }
    let prefix = '';
    if (v < 0) {
        v = Math.abs(v);
        prefix = '-';
    }

    const base = 1024;
    const units = ['B', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB'];
    for (let i = 0, l = units.length; i < l; i++) {
        const min = Math.pow(base, i);
        const max = Math.pow(base, i + 1);
        if (v > min && v < max) {
            const unit = units[i];
            v = prefix + (v / min).toFixed(places) + space + unit;
            break;
        }
    }
    return v;
};

export const BSF = (v, places = 1) => {
    return BF(v, places, ' ');
};

// time
export const TF = (v, space = '') => {
    const ms = toNum(v, true);

    if (ms < 1000) {
        return `${ms}${space}ms`;
    }

    if (ms < 60 * 1000) {
        const ss = Math.floor(ms / 1000);
        const sms = Math.round((ms - ss * 1000) / 100);
        if (sms) {
            return `${ss}.${sms}${space}s`;
        }
        return `${ss}${space}s`;
    }

    const s = Math.round(ms / 1000);

    const m = 60;
    const h = m * 60;
    const d = h * 24;

    if (s < h) {
        const minutes = Math.floor(s / m);
        const seconds = s - minutes * m;
        return `${minutes}${space}m ${seconds}${space}s`;
    }

    if (s < d) {
        const hours = Math.floor(s / h);
        const minutes = Math.floor((s - hours * h) / m);
        const seconds = s - hours * h - minutes * m;
        return `${hours}${space}h ${minutes}${space}m ${seconds}${space}s`;
    }

    const days = Math.floor(s / d);
    const hours = Math.floor((s - days * d) / h);
    const minutes = Math.floor((s - days * d - hours * h) / m);
    const seconds = s - days * d - hours * h - minutes * m;
    return `${days}${space}d ${hours}${space}h ${minutes}${space}m ${seconds}${space}s`;
};

export const TSF = (v) => {
    return TF(v, ' ');
};

export const dateFormat = (date, locale, format) => {
    if (locale) {
        if (format === 'date') {
            return new Intl.DateTimeFormat(locale, {
                year: 'numeric',
                month: 'numeric',
                day: 'numeric'
            }).format(date);
        }
        if (format === 'time') {
            return new Intl.DateTimeFormat(locale, {
                hour: 'numeric',
                minute: 'numeric',
                second: 'numeric'
            }).format(date);
        }
        return new Intl.DateTimeFormat(locale, {
            year: 'numeric',
            month: 'numeric',
            day: 'numeric',
            hour: 'numeric',
            minute: 'numeric',
            second: 'numeric'
        }).format(date);
    }

    if (format === 'date') {
        return new Date(date).toLocaleDateString();
    }

    if (format === 'time') {
        return new Date(date).toLocaleTimeString();
    }

    return new Date(date).toLocaleString();
};
