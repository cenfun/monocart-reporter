...
const { TraceMap, sourceContentFor } = require('../../../runtime/monocart-coverage.js');
...
const initOriginalList = (tracer, fileSources, options) => {

    // source filter
    let sourceList = tracer.sources;
    ...
    for (const sourcePath of sourceList) {
        ...
        const sourceContent = sourceContentFor(tracer, sourcePath);
...
const unpackSourceMap = async (item, state, options) => {
    ...
    const originalMap = initOriginalList(tracer, fileSources, options);
    ...
    item.functions.forEach((block) => {
        block.ranges.forEach((range, index) => {
            ...
            const res = getOriginalRange(tracer, range, generatedPositionMapping, originalMap);
            if (!res) {
                return;
            }
