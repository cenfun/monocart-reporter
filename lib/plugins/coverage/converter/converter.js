...
const decodeSourceMappings = async (sourceMap, generatedPositionMapping) => {
    ...
    // });
    // allDecodedMappings.forEach((item, i) => {
    //     if (item.generatedIndex !== i) {
    //         EC.logRed(item, i);
    //     }
    // });
...
const unpackSourceMap = async (item, state, options) => {
    ...
    Util.logTime(options.debug, `decode source mappings ${distFile}`, time_start_decode);
    ...
    item.functions.forEach((block) => {
        block.ranges.forEach((range, index) => {
            ...
            const result = findOriginalRange(range, allDecodedMappings, generatedPositionMapping, originalMap);
            if (!result) {
                return;
            }

            const {
                originalState, originalStart, originalEnd
            } = result;
    ...
    // Util.logTime(options.debug, `mapping source codes ${distFile}`, time_start_mapping);
