...
// position mapping for conversion between offset and line/column for original source
const PositionMapping = require('./position-mapping.js');
// formatted source mapping
...
const initOriginalList = (consumer, fileSources, options) => {
    ...
    for (const sourcePath of sourceList) {
        ...
        // no need format for source, or manual format on report UI
...
const unpackSourceMap = async (item, state, options) => {
    ...
    item.functions.forEach((block) => {
        block.ranges.forEach((range, index) => {
            ...
            const originalInfo = getOriginalRange(consumer, startOffset, endOffset, positionMapping, originalMap);
            if (!originalInfo) {
                return;
            }
...
const convertV8List = async (v8list, options) => {
    ...
    for (const item of jsList) {
        ...
        const formatMapping = new Mapping(content, mapping, parseLines);
