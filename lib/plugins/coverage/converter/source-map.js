...
const findEndMapping = (allDecodedMappings, endOffset, originalState) => {
    const endIndex = findMappingIndex(allDecodedMappings, endOffset - 1);
    ...
    if (nextMapping.line === endMapping.line) {
        return {
            ... nextMapping,
            originalColumn: nextMapping.originalColumn - 1
        };
    }
...
const getOriginalRange = (allDecodedMappings, range, originalMap) => {
    ...
    // end exclusive
    const endMapping = findEndMapping(allDecodedMappings, endOffset, originalState);
