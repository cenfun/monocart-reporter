...
const findOriginalPosition = (consumer, line, column, direction) => {
...
// start
const findOriginalStartPosition = (consumer, sLoc, generatedPositionMapping, end) => {
    const { line, column } = sLoc;
    const sp = findOriginalPosition(consumer, line + 1, column, 'right');
    // if (sp) {
    //     // check offset if start > end
    //     // ()=>r, o=
    //     // start to end: "()=>r", but found start is: "o" that great than end "r"

    //     const gp = consumer.generatedPositionFor(sp);
    //     if (gp.line !== null) {
...
const findOriginalEndPosition = (consumer, eLoc, generatedPositionMapping, start, end) => {
    ...
    let ep = findOriginalPosition(consumer, line + 1, currentColumn, 'left');
    if (!ep) {
...
const findOriginalEndInRange = (consumer, generatedPositionMapping, start, end) => {
    ...
    for (let i = end - 2; i > start; i--) {
        ...
        const op = findOriginalPosition(consumer, loc.line + 1, loc.column, 'left');
        if (op) {
            return op;
        }
    }
...
const getOriginalRange = (consumer, range, generatedPositionMapping, originalMap) => {
    ...
    const oSLoc = findOriginalStartPosition(consumer, sLoc, generatedPositionMapping, endOffset);
    if (!oSLoc) {
        // not found start
        return;
    }
