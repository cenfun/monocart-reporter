...
const fixStartMapping = (startMapping, range, originalState) => {
    // TODO not exact matched
...
const findNextMapping = (decodedMappings, mapping) => {
    const {
    ...
    while (i < l) {
        ...
        if (item.originalLine !== originalLine || item.originalColumn !== originalColumn) {
            return item;
        }
        i += 1;
    }
...
const fixEndMapping = (endMapping, startMapping, originalState) => {
    const decodedMappings = originalState.decodedMappings;
    const nextMapping = findNextMapping(decodedMappings, endMapping);
...
const getOriginalRange = (allDecodedMappings, range, originalMap) => {
    ...
    // ==================================================================================
    // debug
    // if (originalState.sourcePath.endsWith('institutions-cache.ts') && range.count === 0) {
    ...
    // always success
    fixStartMapping(startMapping, range, originalState);

    const endMapping = findEndMapping(allDecodedMappings, range);
    const fixed = fixEndMapping(endMapping, startMapping, originalState);
    if (!fixed) {
        return;
    }
