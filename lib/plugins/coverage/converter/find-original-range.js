...
const findStartMapping = (range, allDecodedMappings, originalMap) => {
    ...
    return {
        startMapping, originalState
    };
};


const findEndMapping = (range, startMapping, allDecodedMappings, generatedPositionMapping, originalState) => {
...
const findOriginalRange = (range, allDecodedMappings, generatedPositionMapping, originalMap) => {
    ...
    const startResult = findStartMapping(range, allDecodedMappings, originalMap);
    if (!startResult) {
        return;
    }
    ...
    // ==================================================================================
    // debug
    // if (originalState.sourcePath.endsWith('app-routing.module.ts') && range.startOffset === 1984) {
    ...
    const endMapping = findEndMapping(range, startMapping, allDecodedMappings, generatedPositionMapping, originalState);
    if (!endMapping) {
        return;
    }
