...
const addCssRange = (rangesMap, currentSource, originalStart, originalEnd) => {

    let currentRanges = rangesMap[currentSource];
    if (!currentRanges) {
        currentRanges = [];
        rangesMap[currentSource] = currentRanges;
    }
...
const unpackCssSourcemap = async (item, v8list, options) => {
    ...
    // EC.logYellow(item.url);

    item.ranges.forEach((range) => {
        const { start, end } = range;

        const sLoc = generatedMapping.offsetToLocation(start);
        ...
        const eLoc = generatedMapping.offsetToLocation(end);
        ...
        if (!oELoc) {
            ...
            addCssRange(rangesMap, currentSource, originalStart, originalMaxEnd);
        ...
        addCssRange(rangesMap, currentSource, originalStart, originalEnd);

    });

    originalMappings.forEach((originalMapping, currentSource) => {

        const ranges = dedupeRanges(originalMapping.list);
