...
const getNetworkSummary = (log) => {
    ...
    const countWaterfall = (entry, index) => {

        let pageId = entry.pageref;
        if (!pageId) {
            pageId = `page-${index}`;
        }
        if (!waterfalls[pageId]) {
            const fallbackTimestamp = new Date(entry.startedDateTime).getTime();
            waterfalls[pageId] = {
                timestampStart: fallbackTimestamp,
                start: 0,
                time: 0,
                entries: []
            };
            if (entry.pageref) {
                Util.logDebug(`fallback network page created for missing HAR page "${entry.pageref}"`);
            } else {
                Util.logDebug('fallback network page created for HAR entry without pageref');
            }
        }

        const pageWaterfall = waterfalls[pageId];
