...
const getServerUrl = (serverOptions = {}) => {
    return `ws://${serverOptions.host}:${serverOptions.port}`;
};

// ===============================================================================

let client;
const useState = (testInfo, clientOptions = {}) => {
    if (!client) {
        const reporterOptions = Util.resolveReporterOptions(testInfo);
        const options = Util.mergeOption(defaultOptions, reporterOptions.state);
