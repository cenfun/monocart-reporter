...
const defaultStateOptions = {
    ...
    // onMessage: async function(ws, message, next) {
    //     await next();
    // },

    // onClose: async function(data, config) {
    // },
...
const onMessage = async (ws, buf, options) => {
    ...
    // handler depends on custom onMessage
    const next = () => {
    ...
    if (typeof options.onMessage === 'function') {
        await options.onMessage.call(options, ws, message, next);
    } else {
        next();
    }
