    ...
    */
    ...
    async testStepHandler(steps, caseItem) {
        ...
        for (const testStep of steps) {
            ...
            // step-level attachments (testInfo.attach inside test.step)
            // expose name/contentType only; file copy/rewrite is handled at case level
            if (Util.isList(testStep.attachments)) {
                step.attachments = testStep.attachments.map((a) => ({
                    name: a.name,
