    ...
    */
    ...
    testCaseHandler(testCase, list, parent) {
        ...
        testCase.results.forEach((testResult) => {
            ...
            result.stderr = result.stderr.concat(testResult.stderr);
            result.stdout = result.stdout.concat(testResult.stdout);
            ...
            result.subs = result.subs.concat(this.testStepHandler(testResult.steps));
    ...
    caseErrorsHandler(caseItem) {
        ...
        if (caseItem.outcome === 'unexpected') {
            caseItem.errors = [{
                message: `Expected status is "${caseItem.expectedStatus}" not "${caseItem.status}"`
            }];
            return;
        }
