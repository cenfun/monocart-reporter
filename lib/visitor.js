...
class Visitor {
    ...
    async getData() {
        ...
        if (typeof handler === 'function') {
            ...
            this.getCustomVisitors(defaultColumns, customVisitors);
    ...
    getCustomVisitors(list, customVisitors) {
        ...
        list.forEach((column) => {
            ...
            if (Util.isList(column.subs)) {
                this.getCustomVisitors(column.subs, customVisitors);
            }
        });
    }
    ...
    async customVisitorsHandler(data, metadata) {
        ...
        if (this.customVisitors) {
            for (const item of this.customVisitors) {
                data[item.id] = await item.visitor.call(this, data, metadata, collect);
            }
        }
    }
