    ...
    */
    ...
    attachmentsHandler(caseItem, caseId) {
        ...
        attachments.forEach((item, i) => {
            ...
            // before path change
            this.coverageReportHandler(item);
    ...
    coverageReportHandler(item) {
        if (item.name !== 'coverage' || item.contentType !== 'text/html') {
            return;
        }

        const jsonPath = path.resolve(path.dirname(item.path), 'coverage-summary.json');
        if (!fs.existsSync(jsonPath)) {
            return;
        }

        const summary = Util.readJSONSync(jsonPath);
