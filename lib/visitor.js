...
const { generateCoverageReport } = require('./helper/coverage.js');
...
class Visitor {
    ...
    */
    ...
    artifactHandler(p) {
        ...
        if (!json) {
            EC.logRed(`failed to read artifact: ${p}`);
            return;
        }
        const artifactList = this.artifacts[json.type];
        if (artifactList) {
            artifactList.push(json.data);
        } else {
            this.artifacts[json.type] = [json.data];
        }
    }

    async artifactsHandler() {
        const coverage = this.artifacts.coverage;
        this.artifacts = null;
        if (coverage) {
