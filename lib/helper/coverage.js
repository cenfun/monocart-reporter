...
const saveV8HtmlReport = async (reportData, coverageDir) => {
    ...
    // copy 2 js lib
    const v8LibFile = 'monocart-coverage-v8.js';
    const runtimeFiles = [v8LibFile, 'devtools-formatter-worker.js'];
    runtimeFiles.forEach((file) => {
        const jsPath = path.resolve(__dirname, `../runtime/${file}`);
        if (fs.existsSync(jsPath)) {
            const toPath = path.resolve(coverageDir, file);
            fs.copyFileSync(jsPath, toPath);
        } else {
            EC.logRed(`[MCR] not found runtime lib: ${Util.formatPath(jsPath)}`);
        }
    });

    // save html
    const content = `<script src="./${coverageDataFile}"></script><script src="./${v8LibFile}"></script>`;
    ...
    await Util.writeFile(path.resolve(coverageDir, 'index.html'), html);
...
const generateV8Report = async (list, testInfo, options, coverageDir) => {
    ...
    await saveV8HtmlReport(htmlReport, coverageDir);
