...
const saveV8SummaryIndex = (testInfo, report, coverageDir, template) => {
    const indexPath = path.resolve(coverageDir, 'index.html');

    const title = `Coverage Report - ${testInfo.title}`;
    ...
    Util.writeFileSync(indexPath, html);
...
const generateV8Report = async (list, testInfo, options, coverageDir) => {
    ...
    for (const item of list) {
        ...
        const title = `Coverage Report - ${item.dirname}/${item.filename}`;
        const html = Util.replace(template, {
            title,
            content
        });

        const fileDir = path.resolve(coverageDir, item.dirname);
        const reportPath = path.resolve(fileDir, `${item.filename}.html`);
        item.reportPath = reportPath;
        await Util.writeFile(reportPath, html);
    ...
    saveV8SummaryIndex(testInfo, report, coverageDir, template);
