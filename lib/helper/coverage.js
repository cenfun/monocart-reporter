...
// ========================================================================================================
const initV8List = (v8list) => {
    // filter no source or text and init type extname
    return v8list.filter((item) => {
        if (item.source) {
            item.type = 'js';
            item.extname = '.js';
            return true;
...
const generateV8Coverage = async (v8list, testInfo, options) => {
    ...
    v8list.forEach((item, i) => {
        ...
        fileSources[sourcePath] = item.source || item.text;
    ...
    if (options.toIstanbul) {
        report = await toIstanbulReport(v8list, testInfo, options, fileSources);
    } else {
