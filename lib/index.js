...
class MonocartReporter {
    ...
    constructor(userOptions = {}) {
        ...
        getTrends(this.options.trend).then((trends) => {
            ...
            Util.logError(`failed to read trends: ${(e && e.message) || e}`);
        });
    ...
    init() {
        ...
        this.options.outputDir = path.dirname(outputFile);
    ...
    onBegin(config, suite) {
        ...
        if (this.options.copyAttachments) {
            ...
            // console.log(this.options.testOutputDir);
            if (this.options.outputDir === this.options.testOutputDir) {
                Util.logInfo('copyAttachments disabled: outputDir is same as testOutputDir, attachments are already in place.');
            } else {
                this.options.attachmentsDir = path.resolve(this.options.outputDir, 'attachments');
            }
        }
    ...
    // Called immediately before test runner exits, after all reporters have received onEnd.
    // Suitable for uploading reports or other post-completion side effects.
    async onExit() {
        ...
        try {
            await onExit(this.options);
        } catch (e) {
            Util.logError(`onExit handler failed: ${(e && e.message) || e}`);
        }
    }
