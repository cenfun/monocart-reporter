...
const testCaseHandler = (testCase, list, parent) => {
    parent.subType = 'case';
    //When a single test(title, testFunction) is running in multiple projects 
    //or repeated multiple times(retry), 
    //it will have multiple TestCase objects in corresponding projects' suites.
...
const generateReport = (root, config) => {
    ...
    let filename = reportOption.outputFilename;
    if (path.extname(filename) === '.html') {
        filename = filename.substr(-5);
    }

    const jsonPath = path.resolve(reportOption.outputFolder, 'grid-report.json');
    Util.writeJSONSync(jsonPath, reportData, true);
    console.log(`saved json report: ${EC.cyan(Util.relativePath(jsonPath))}`);
    
    const reportDistPath = path.resolve(__dirname, '../packages/report/dist/playwright-report-grid.js');
    ...
    const htmlPath = path.resolve(reportOption.outputFolder, 'grid-report.html');
