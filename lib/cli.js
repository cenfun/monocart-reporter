...
const createServer = (app, options) => {
    ...
    if (options.ssl) {
        ...
        const resolvedKey = keyPath && path.resolve(keyPath);
        const resolvedCert = certPath && path.resolve(certPath);
        if (!resolvedKey || !fs.existsSync(resolvedKey)) {
            EC.logRed(`SSL key file not found: ${keyPath}`);
            exit(1);
            return;
        }
        if (!resolvedCert || !fs.existsSync(resolvedCert)) {
            EC.logRed(`SSL cert file not found: ${certPath}`);
            exit(1);
            return;
        }
        const serverOptions = {
...
const parseJsonConfig = (configPath) => {
    const content = Util.readFileSync(configPath);
    try {
        return JSON.parse(content);
    } catch (e) {
        Util.logError(`ERROR: failed to parse config "${configPath}": ${e.message}`);
    }
};
